...
/*
  abstract class DeliteOpFilterReduceFold[A:Manifest,R:Manifest]
    ...
    def accum: Exp[A] => Exp[R] //= (elem) => zero // given an element, return appropriate accum (i.e. vec of size)
  ...
  def emitCollectElem(op: AbstractFatLoop, sym: Sym[Any], elem: DeliteCollectElem[_,_], prefixSym: String = "")(implicit stream: PrintWriter) {
    if (elem.cond.nonEmpty) {
      ...
      if (deliteKernel)
        ...
        stream.println("{")
        stream.println(prefixSym + quote(sym) + "_data(" + quote(sym) + "_size) = " + quote(getBlockResult(elem.func)))
    ...
    (symList zip op.body) foreach {
      case (sym, elem: DeliteCollectElem[_,_]) =>
       /* if (elem.cond.nonEmpty) {
        ...
        /*stream.println("val " + quote(sym) + "_data : Array[" + remap(getBlockResult(elem.func).Type) + "] = {"/*}*/)
