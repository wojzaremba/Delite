...
trait DataTableOpsExp extends DataTableOps with BaseFatExp { this: DataTableOpsExp with OptiQLExp =>
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer): Exp[A] = (e match {
    ...
    case DataTableSize(t) => dataTableSize(f(t))
    case Reflect(DataTableObjectApply(m,x), u, es) => reflectMirrored(Reflect(DataTableObjectApply(m,f(x)), mapOver(f,u), f(es)))(mtype(manifest[A]))
    case Reflect(DataTablePrintAsTable(t), u, es) => reflectMirrored(Reflect(DataTablePrintAsTable(f(t)), mapOver(f,u), f(es)))(mtype(manifest[A]))
